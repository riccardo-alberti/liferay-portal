...
import com.liferay.commerce.qualifier.configuration.AddressQualifierConfiguration;
 ...
 */
@Component(
	configurationPid = "com.liferay.commerce.qualifier.configuration.AddressQualifierConfiguration",
	enabled = false, immediate = true,
	service = {CommerceQualifierMetadata.class, ModelListener.class}
)
public class CommerceShippingAddressQualifierMetadata
	extends BaseCommerceQualifierMetadata<Address> {

	@Override
	public Class<?> getConfigurationBeanClass() {
		return AddressQualifierConfiguration.class;
	}

	@Override
	public Predicate getFilterPredicate() {
		ListType accountEntryAddressTypeShipping =
			_listTypeLocalService.getListType(
				AccountListTypeConstants.ACCOUNT_ENTRY_ADDRESS_TYPE_SHIPPING,
				AccountListTypeConstants.ACCOUNT_ENTRY_ADDRESS);

		ListType accountEntryAddressTypeBillingAndShipping =
			_listTypeLocalService.getListType(
		...
		return AddressTable.INSTANCE.typeId.in(
			new Long[] {
				accountEntryAddressTypeShipping.getListTypeId(),
				accountEntryAddressTypeBillingAndShipping.getListTypeId()
			});
	}
